...
from tkinter import filedialog  # Importe la boîte de dialogue pour choisir le fichier
...
class MainWindow:
    ...
    def create_widgets(self):
        ...
        define_budget_button.grid(row=2, column=0, columnspan=2, padx=10, pady=3, sticky=(tk.W, tk.E))
        
        # Bouton pour exporter vers Excel
        export_button = ttk.Button(
            main_frame,
            text="Exporter vers Excel",
            command=self.export_to_excel,  # Ajoute la commande pour l'exportation
            style="TButton",
        )
        export_button.grid(
            row=6, column=0, columnspan=2, padx=5, pady=5, sticky=(tk.W, tk.E)
        )

        # Zone d'affichage du budget
        budget_group = ttk.LabelFrame(main_frame, text="Résumé du Budget", padding=10)
        budget_group.grid(row=4, column=0, columnspan=2, pady=20, sticky=(tk.W, tk.E))

        self.budget_label = ttk.Label(budget_group, text="Solde actuel :", style='TLabel')
        self.budget_label.grid(row=0, column=0, sticky=tk.W)
        self.budget_amount = ttk.Label(
            budget_group, text="0.00 €", style='TLabel', font=('Segoe UI', 12, 'bold')
        )
        self.budget_amount.grid(row=0, column=1, sticky=tk.E)

        # Affichage du budget prévisionnel et des dépassements
        self.prevision_label = ttk.Label(
            budget_group, text="Budget Prévisionnel :", style="TLabel"
        )
        self.prevision_label.grid(row=1, column=0, sticky=tk.W)
        self.prevision_amount = ttk.Label(
            budget_group, text="0.00 €", style="TLabel", font=("Segoe UI", 12, "bold")
        )
        self.prevision_amount.grid(row=1, column=1, sticky=tk.E)

        self.depassements_label = ttk.Label(
            budget_group, text="Dépassements :", style="TLabel"
        )
        self.depassements_label.grid(row=2, column=0, sticky=tk.W)
        self.depassements_text = tk.Text(
            budget_group, height=3, width=30, font=("Segoe UI", 10)
        )
        self.depassements_text.grid(row=2, column=1, sticky=tk.E)
        self.depassements_text.config(
            state=tk.DISABLED
        ...
        self.graphics_notebook.grid(row=5, column=0, columnspan=2, pady=17, sticky=(tk.W, tk.E, tk.N, tk.S))
        ...
        self.graphics_notebook.add(
            self.revenus_depenses_frame, text="Revenus/Dépenses"
        )
    ...
    def update_budget_display(self):
        ...
        if budget_previsionnel:
            total_budget_previsionnel = sum(
                budget_previsionnel.montants_par_categorie.values()
            )
        ...
        if depassements:
            depassements_str = "\n".join(
                f"{categorie}: {depassement:.2f} €"
                for categorie, depassement in depassements.items()
            )
    ...
    def update_transactions_display(self):
        ...
        for revenu in self.budget_manager.get_all_revenus():
            self.revenues_list.insert(
                tk.END,
                f"{revenu.date.strftime('%d/%m/%Y')} - +{revenu.montant:.2f}€ ({revenu.source})",
            )
        ...
        for depense in self.budget_manager.get_all_depenses():
            self.expenses_list.insert(
                tk.END,
                f"{depense.date.strftime('%d/%m/%Y')} - -{depense.montant:.2f}€ ({depense.categorie})",
            )
    ...
    def update_graphics(self):
        ...
        for child in self.revenus_depenses_frame.winfo_children():
            child.destroy()
        afficher_graphique_depenses_par_categorie(
            self.depenses_frame, self.budget_manager
        )
        afficher_graphique_revenus_depenses(
            self.revenus_depenses_frame, self.budget_manager
        )
    ...
    def export_to_excel(self):
        """
        Ouvre une boîte de dialogue pour choisir le nom et l'emplacement du fichier Excel,
        puis appelle la méthode d'exportation du BudgetManager.
        """
        ...
        if filename:  # Vérifie si un nom de fichier a été fourni
            self.budget_manager.exporter_vers_excel(filename)
